...
if (!function_exists('checkProductIsCreated')) {
 ...
 function get_metal_image($id='', $metal='', $thumbnail_image='')
 {
    $attribute_value = check_attribute_value($metal);
    //echo '<pre>';print_r($metal);die;
    $product_metal_image = ProductVariantImage::where('variant', $attribute_value)->where('product_id', $id)->first();
    $met_photos = (isset($product_metal_image->image) && !empty($product_metal_image->image)) ? explode(',', $product_metal_image->image) : array() ;
    $photos = (isset($met_photos[0]) && !empty($met_photos[0])) ? $met_photos[0] : '' ;
    return $photos;
 }

    function TaxRate($zip_code='')
    {
            //echo $zip_code;die;
            $curl = curl_init();

                    curl_setopt_array($curl, [
                    CURLOPT_URL => "https://retrieveustaxrate.p.rapidapi.com/GetTaxRateByZip?zip=$zip_code",
                    ...
                    CURLOPT_HTTPHEADER => [
                        ...
                        "X-RapidAPI-Key: 29b97b4fb4msh112a0666d5081b5p1c7cd2jsn34b2cd233570"
                        ],
                    ]);
...
if( ! function_exists('detectUserAgentDevice')){
	 function detectUserAgentDevice(){
		
		$device_type = "desktop";
		
		$tablet_browser = 0;
		$mobile_browser = 0;
		$is_mobile_and_tablet = 0;
		...
		//echo $tablet_browser.'==>'.$mobile_browser.'===>'.
		if ($tablet_browser > 0) {
...
if( ! function_exists('get_product_image')){
	 function get_product_image($product_thumb_img = "", $device_type = "",$new_folder_name = "mobile"){
		
		$product_image = "";
		$imageFolder = array('product_gallery_images'=>'product_gallery_images','product_gallery_zoom_images'=>'product_gallery_zoom_images','mobile'=>'mobile');
		if(!empty($product_thumb_img)){
			$product_image = uploaded_asset($product_thumb_img);
			$original_image = $product_image;
			if($device_type != "mobile" && $new_folder_name == "product_gallery_images" && $new_folder_name == "product_gallery_zoom_images"){
				$product_image = $original_image;
			}else{
				/*if($new_folder_name == "product_gallery_images"){
					$product_image = str_replace('/all/','/product_gallery_images/',$product_image);
				}elseif($new_folder_name == "product_gallery_zoom_images"){
					$product_image = str_replace('/all/','/product_gallery_zoom_images/',$product_image);
				}else{
					$product_image = str_replace('/all/','/mobile/',$product_image);
				}*/
				
				if($new_folder_name == "mobile"){
				...
				$product_image = str_replace('/all/','/'.$new_folder_name.'/',$product_image);
				
				
				
				$thumb_img_path = ($_SERVER['HTTP_HOST'] == "localhost") ? str_replace('https://localhost/retail/public/','',$product_image) : str_replace('https://bautlr.com/public/','',$product_image);
				
				$file_full_path = public_path().'/'.$thumb_img_path;
				
				$file_ext = pathinfo($file_full_path, PATHINFO_EXTENSION);
				$webp_file_full_path = str_replace('.'.$file_ext,'.webp',$file_full_path);
				
				if(file_exists($webp_file_full_path)){
					$product_image = str_replace('.'.$file_ext,'.webp',$product_image);
...
/*if (!function_exists('restrictAccessByCountry')) {
    function restrictAccessByCountry(){
        $country = 0;
