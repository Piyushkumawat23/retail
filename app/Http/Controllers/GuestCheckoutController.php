...
class GuestCheckoutController extends Controller
{
        ...
        if ($request->payment_option != null) {
            ...
            if ($request->session()->get('combined_order_id') != null) {
                ...
                if (class_exists($decorator)) {
                    ...
                    return redirect()->route('guest_order_confirmed');
                }
            }
        } else {
    ...
    public function checkout_done($combined_order_id, $payment)
    {
        ...
        //echo '<pre>';print_r($combined_order);die;
    ...
    { 
        ...
        //echo '<pre>';print_r($carts);die('hie');
        ...
        $carts_del_info = Cart::where('temp_user_id', $user_id)
                ->get();
        ...
        if ($carts_del_info && count($carts_del_info) > 0) {
            ...
            return view('frontend.guest_shipping_info', compact('categories','carts','carts_del_info','shipping_info', 'total'));
        ...
        {   
            //$gettaxrate=TaxRate($request->postal_code);
            //$tax_rate = !empty($gettaxrate) ? json_decode($gettaxrate) : '';
            $tax_rate_value = 0;//isset($tax_rate->TaxRate) ? $tax_rate->TaxRate : 0;
            $gtr = array('res'=>1,'tax_rate_value'=>$tax_rate_value);
            echo json_encode($gtr); die;
        }  
        public function guestTotal(Request $request)
        {

                Cart::where('id', $request->cart_id)
                        ->update(
                                [
                                    'tax' => $request->guest_tax_amount,
                                    'shipping_cost' => $request->ship
                                ]
                );
        ...
        foreach ($states as $state) {
            $html .= '<option value="' . $state->id . '" shipping="' . $state->normal_shipping_cost . '">' . $state->name . '</option>';
        }
   ...
   public function store_shipping_info(Request $request)
    {   
        ...
        if(User::where('email', $request->email)->first() != null){
            $newUser  = User::select('id')->where('email', $request->email)->first();
            ...
            $address->phone         = (isset($request->phone) && !empty($request->phone) ) ? $request->phone : '';
            $address->save();
        ...
        $carts = Cart::where('user_id', $newUser->id)->get();
       // echo '<pre>';print_r($newUser->id);
        ...
        //echo '<pre>';print_r($carts->shipping_cost);
        if(get_setting('minimum_order_amount_check') == 1){
            $subtotal = 0;
            foreach (Cart::where('user_id', $newUser->id)->get() as $key => $cartItem){
                $subtotal += $cartItem['price'] * $cartItem['quantity'];
            }
            if ($subtotal < get_setting('minimum_order_amount')) {
                flash(translate('You order amount is less then the minimum order amount'))->warning();
        ...
        if ($request->payment_option != null) {
            ...
            if ($request->session()->get('combined_order_id') != null) {
                //echo '<pre>';print_r( $request->payment_option);die;
                // If block for Online payment, wallet and cash on delivery. Else block for Offline payment
                $decorator = __NAMESPACE__ . '\\Payment\\' . str_replace(' ', '', ucwords(str_replace('_', ' ', $request->payment_option))) . "Controller";

                //echo '<pre>';print_r( $decorator);die;
                if (class_exists($decorator)) {
                    return (new $decorator)->pay($request);
                }
                else {
    ...
    public function order_confirmed()
    {
        ...
        Session::put('user_id', $combined_order->user_id);
