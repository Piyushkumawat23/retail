...
class HomeController extends Controller
{
    ...
    {   
       /* $ip = $_SERVER['REMOTE_ADDR'];
        ...
        echo '<pre>'; print_r($details->postal); // -> "US"
        die;*/
        ...
        $featured_products = Product::where('featured',1)->where('published', 1)->limit(10)->get();

        return view('frontend.index', compact('featured_categories', 'todays_deal_products', 'newest_products','featured_products'));
    }
    
    public function restrictedCountry()
    {   
        //die('hello');
         return view('frontend.restrict');
    }
    ...
    public function login(Request $request)
    {
        ...
        return view('frontend.user_login', compact('page'));
    }
    ...
    public function aboutus(){
        $page =  Page::where('type', 'custom_page')->where('slug','about-us')->first();
        return view("frontend.policies.aboutus", compact('page'));
    } 

    public function services(){
        $page =  Page::where('type', 'custom_page')->where('slug','services')->first();
        return view("frontend.policies.services", compact('page'));
    }
...
public function cron_resize_images(){
	
    $this->resize_product_main_hover_images();

	$this->resize_product_main_images();
	
	$this->resize_product_gallery_images();
	
	die('passss images');
}

public function resize_product_main_hover_images(){
    
    $uploaded_images = DB::select('SELECT u.id, u.file_name FROM uploads u INNER JOIN products p ON p.hover_img = u.id WHERE  u.`is_resized_img` = 0 GROUP BY u.id, u.file_name HAVING COUNT(*) = 1 ');
    
    if(!empty($uploaded_images)){
        foreach($uploaded_images as $uploaded_image){
            
            
            DB::update('UPDATE uploads SET is_resized_img = 1 WHERE id = '.$uploaded_image->id);
            
            $file_full_path = public_path().'/'.$uploaded_image->file_name;
            
            if(file_exists($file_full_path)){
                
                $this->resize_image($file_full_path,$uploaded_image->file_name,'main_image', 300);
                $this->resize_image($file_full_path,$uploaded_image->file_name, 'main_image_desktop', 500);
                
            }else{
                echo '<br/>file not exit: '.$uploaded_image->file_name;
            }
            
            /*$upload = Upload::findOrFail($uploaded_image->id);
            $upload->is_resized_img = 1;
            $upload->save();*/
            
        }
    }else{
        echo 'No any product main images for resize';
    }
}

public function resize_product_main_images(){
	
	$uploaded_images = DB::select('SELECT u.id, u.file_name FROM uploads u INNER JOIN products p ON p.thumbnail_img = u.id WHERE  u.`is_resized_img` = 0 GROUP BY u.id, u.file_name HAVING COUNT(*) = 1 ');
...
public function resize_product_gallery_images(){
	
	$uploaded_images = DB::select('SELECT u.id, u.file_name FROM uploads u INNER JOIN products p ON FIND_IN_SET(u.id, p.photos) > 0 WHERE  u.is_resized_gallery_img = 0 GROUP BY u.id, u.file_name HAVING COUNT(*) = 1;');
	
	//echo '<pre>resize_product_gallery_images'; print_r($uploaded_images); die;
	if(!empty($uploaded_images)){
		foreach($uploaded_images as $uploaded_image){
			
			DB::update('UPDATE uploads SET is_resized_gallery_img = 1 WHERE id = '.$uploaded_image->id);
			
			$file_full_path = public_path().'/'.$uploaded_image->file_name;
			
			if(file_exists($file_full_path)){
				
				$this->resize_image($file_full_path,$uploaded_image->file_name, 'gallery_images', 150);
				$this->resize_image($file_full_path,$uploaded_image->file_name, 'gallery_zoom_images', 500);
				
			}
			
			/*$upload = Upload::findOrFail($uploaded_image->id);
			$upload->is_resized_gallery_img = 1;
			$upload->save();*/
			
		}
	}else{
		echo 'No any product gallery images for resize';
	}
}


	public function resize_image($file_path = "",$file_name = "", $image_type = "main_image", $image_size = 300){
		
		if(!empty($file_path)){
			
			$file_ext = pathinfo($file_path, PATHINFO_EXTENSION);
			$file_ext = strtolower(strtoupper($file_ext));
			...
			if(in_array($file_ext, array('jpg','jpeg','png'))){
				...
				// Resize the original image to the new dimensions and copy it into the new image
				imagecopyresampled($new_image, $original_image, 0, 0, 0, 0, $new_width, $new_height, $width, $height);
				
				// Save the new image as a JPG
				$new_folder_name = '/mobile/';
				
				if($image_type ==  "gallery_images"){
					$new_folder_name = '/product_gallery_images/';
				}elseif($image_type ==  "gallery_zoom_images"){
					$new_folder_name = '/product_gallery_zoom_images/';
				}elseif($image_type == "main_image_desktop"){
					$new_folder_name = '/product_desktop_images/';
				}
				
				if($file_ext == "png"){
					imagepng($new_image, str_replace('/all/',$new_folder_name,$file_path));
				}else{
					imagejpeg($new_image, str_replace('/all/',$new_folder_name,$file_path), 90);
				}
				...
				//if($image_type == "main_image"){
					...
					$resized_image_path = str_replace('/all/',$new_folder_name,$file_path);
					
					if(file_exists($resized_image_path)){
						
						$webp_image_path = str_replace('.'.$file_ext,'.webp',$resized_image_path);
						
						$this->convertImageToWebP($resized_image_path,$webp_image_path,$file_ext);
					}
...
public function convertImageToWebP($source, $destination,$file_ext) {
    ...
    imagewebp($image, $destination, 90); // 80 is the quality, you can adjust as needed
