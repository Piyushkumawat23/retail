			...
			<div class="card-body">
				<form action="{{ route('business_settings.update') }}" method="POST" enctype="multipart/form-data">
					...
					<div class="border-top pt-3">
					    <div class="form-group row">
					        <label class="col-md-3 col-from-label">Top Banner Video</label>
					        <div class="col-md-8">
					            <div class="input-group" data-toggle="aizuploader" data-type="video"> <!-- Change data-type to "video" -->
					                <div class="input-group-prepend">
					                    <div class="input-group-text bg-soft-secondary font-weight-medium">{{ translate('Browse') }}</div>
					                </div>
					                <div class="form-control file-amount">{{ translate('Choose File') }}</div>
					                <input type="hidden" name="types[]" value="topbar_banner_video"> <!-- Change name attribute -->
					                <input type="hidden" name="topbar_banner_video" class="selected-files" value="{{ get_setting('topbar_banner_video') }}"> <!-- Change name attribute -->
					            </div>
					            <div class="file-preview"></div>
					        </div>
					    </div>
					    <div class="form-group row">
					        <label class="col-md-3 col-from-label">{{translate('Topbar Banner Video Link')}}</label> <!-- Change label text -->
					        <div class="col-md-8">
					            <div class="form-group">
					                <input type="hidden" name="types[]" value="topbar_banner_video_link"> <!-- Change name attribute -->
					                <input type="text" class="form-control" placeholder="{{ translate('Link with') }} http:// {{ translate('or') }} https://" name="topbar_banner_video_link" value="{{ get_setting('topbar_banner_video_link') }}"> <!-- Change name attribute -->
					            </div>
					        </div>
					    </div>
					</div>
