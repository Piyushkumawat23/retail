      ...
      <div class="card">
            ...
            <div class="card-body p-0">
                <form class="p-4" action="{{ route('states.update', $state->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group mb-3">
                      <label for="name">{{translate('Standard Cost')}}</label>
                      <input type="number" min="0" step="0.01" placeholder="{{translate('Cost')}}" name="normal_shipping_cost" class="form-control" value="{{ $state->normal_shipping_cost }}" required>
                  </div>
                    
                  <div class="form-group mb-3">
                      <label for="name">{{translate('Priority Cost')}}</label>
                      <input type="number" min="0" step="0.01" placeholder="{{translate('Cost')}}" name="shipping_cost" class="form-control" value="{{ $state->shipping_cost }}" required>
                  </div>
